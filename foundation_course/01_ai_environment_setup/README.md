...
cp .env.example .env
python foundation_course/01_ai_environment_setup/tools/check_keys.py

# 2. Cloud LLM (needs GOOGLE_API_KEY)
python foundation_course/01_ai_environment_setup/llm/cloud/chat_gemini.py

# 3. Local LLM (first run downloads the model; needs torch/transformers)
python foundation_course/01_ai_environment_setup/llm/local/chat_local.py
```
